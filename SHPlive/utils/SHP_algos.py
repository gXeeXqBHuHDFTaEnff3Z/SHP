...
def isPOI(pkt, poi, port, subnet, ignored):
    ...
    try:
        ...
        if poi == 'broadcast_domain':
            return check_broadcast_domain_v2(pkt, subnet)
    except Exception as e:
...
STATIC_BPF_FILTERv1 = "(ether dst ff:ff:ff:ff:ff:ff) or " \
             "(ip dst 10.255.255.255) or " \
             "(ip[16:2] & 0xFFFF = 0xFFFF) or " \
             "(ip6 multicast) or " \
             "(ether[0] & 1 != 0)"

STATIC_BPF_FILTERv2 = "(arp or ip broadcast or ip multicast or ip6 multicast or (ether[0] & 1 != 0))"
        
def check_broadcast_domain_v1(pkt, subnet):
...
def check_broadcast_domain_v2(pkt, subnet):
    """
    Return True iff *pkt* would be visible to most or all nodes on the given
    LAN (i.e. matches the broadcast-domain filter defined above).

    Parameters
    ----------
    pkt : scapy.packet.Packet
        A decoded Scapy packet (sniffed or crafted).
    subnet : str | ipaddress.IPv4Network
        The IPv4 network of the capture interface, e.g. "192.168.1.0/24".
        Used to recognise subnet-directed broadcasts.

    Notes
    -----
    *Subnet* only affects IPv4 directed broadcasts; all other tests are
    independent of interface configuration.
    """
    # Normalise subnet argument
    ...
    if Ether in pkt:
        ...
        group_bit = int(dst_mac.split(":")[0], 16) & 0x01
        if group_bit:
            return True
    ...
    if IP in pkt:
        ...
        if dst_ip == IPv4Address("255.255.255.255"):
            return True

        # Directed broadcast for the supplied subnet
        if dst_ip == subnet.broadcast_address:
            return True
    ...
    # ---- IPv6 --------------------------------------------------------------
    if IPv6 in pkt:
        dst_ip6 = IPv6Address(pkt[IPv6].dst)
