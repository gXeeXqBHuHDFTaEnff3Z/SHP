...
csv_stats_file = "statistics_cs.csv"
...
class RepeatingTimer(object):
    ...
    def callback(self):
        ...
        # Create and start a new timer after executing the callback
...
def getHashValue(input):
    try:
        hash_val = hashlib.sha512(input)
    except TypeError:
        hash_val = hashlib.sha512(input.encode('utf-8'))
    return hash_val
...
def getInputValues(packet):
    ...
    if robust:
        ...
        timestamp_val = str(int(time.time()/10))
    if IPv6 in packet:
        string = str(packet[IPv6].src) + timestamp_val
    elif IP in packet:
        string = str(packet[IP].chksum) + timestamp_val
    elif ARP in packet:
        string = str(packet[ARP].pdst) + str(packet[ARP].psrc) + timestamp_val
    #print(f'{time.time()} [DEBG] time:{timestamp_val}, string:{string}')
    return string
        ...
        if encoding_method.startswith("trivial"):
            if ba_curr == sniffed_hash and not isCritFraction:
                if robust:
                    ...
                    dyst_match_count += 1            # << Increment DYST match counter
                    cm_array_current += 1
                    ba_curr = getStringToBinary(cm_array[cm_array_current])
                    print("Found correct hash")
                    sendSignal('arp', signal_arp, 0, 0)
                    sig_send = True
                    update_csv_stats()  # <-- Update CSV after successful match
            else:
                print(f"NoMatch; total matches so far: {str(dyst_match_count)} / {str(pkt_interest_count)}")
...
def cr_sniffing(packet):
    ...
    if isSignal(packet, 'arp'):
        ...
        # write statistics and message to file
        dyst_match_count += 1
...
try:
    if mode == 'cs':
        sniff(iface=interface, prn=cs_sniffing, stop_filter=stop_filter)
    elif mode == 'cr':
        sniff(iface=interface, prn=cr_sniffing, stop_filter=stop_filter)
except KeyboardInterrupt:
    stop_sniff = True  # This will cause the stop_filter to eventually return True
    print("\n--- Final Statistics ---")
    print("Packets of interest seen:", pkt_interest_count)
    print("DYST matches:", dyst_match_count)
    sys.exit(0)
